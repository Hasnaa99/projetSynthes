...
<x-master title="Acceuil">
    <div class="container vh-100">
        ...
        <p class="text-center m-4" style="color: #70c927;font-weight:500">{{ Auth::user()->prenom }}
            {{ Auth::user()->nom }} (Matricule : {{ Auth::user()->codeF }})</p>
        <h2 class="text-center mt-4" style="color: #073515;font-weight:600">Choix de groupe</h2>

        <div class="text-center mt-4">
            <form action="{{ route('acceuil') }}" method="GET">
                <select name="groupe" class="form-select form-select-lg mb-2 mx-auto" style="width: 600px;"
                    aria-label=".form-select-lg example">
                    <option value="">Sélectionner un groupe</option>
                    @foreach ($groupes as $groupe)
                        <option value="{{ $groupe->codeG }}"
                            {{ $selectedGroup && $selectedGroup->codeG == $groupe->codeG ? 'selected' : '' }}>
                            {{ $groupe->codeG }}
                        </option>
                    @endforeach
                </select>
        ...
        @if ($selectedGroup)
            <h2 class="text-center mt-4 mb-4" style="color: #073515;">Modules associés au groupe
                {{ $selectedGroup->codeG }}</h2>
            <ul class="list-group w-50 mx-auto">
                @forelse ($modules as $module)
                    <li class="list-group-item">{{ $module->intitule }}</li>
                @empty
